...
public class UserDetailServiceBean extends GenericEntityServiceImpl<UserDetail, Long> implements UserDetailService {
    ...
    public UserDetail saveLoanAmount(Long userDetailId, UserDetail entity) {
	    	...
	    	userDetail.setTenure(entity.getTenure());
...
//    		}
    ...
    public void interactWithTPSystems(Long userDetailId) throws IOException {
    		//breDAO.invokeBRE(null,null);
		updatePAN(userDetailId);
		
		UserDetail entity = findById(userDetailId);
		if(entity.getProfessionalDetail().getPanVerified() != null && entity.getProfessionalDetail().getPanVerified()) {
			//Skip first CP CALL if CP ref. number already exists.
			if(StringUtils.isBlank( entity.getCpReferanceNumber())) {
				createCPAccount(userDetailId);
				//save(entity);
			...
			breAndCRM(userDetailId);
			
			applicaitonNotification(userDetailId);
		}
		
	}

    @Transactional
	public void breAndCRM(Long userDetailId) throws IOException {
		UserDetail entity = findById(userDetailId);
		//verify CIBIL
		...
		//invoke BRE
		InvokeBREResponse invokeBREResponse = breDAO.invokeBRE(fetchOutputNewApplicationXML,entity);
		String breStatus = extractStatus(invokeBREResponse.getInvokeBREReturn());
		entity.setBreStatus(breStatus);
		save(entity);
		
		String leadKey = createCrmEntity(entity.getId());
    ...
    @Transactional
	public void updatePAN(Long userDetailId) {
		UserDetail entity = findById(userDetailId);
		...
		if(entity.getProfessionalDetail().getPanVerified() == null || !entity.getProfessionalDetail().getPanVerified()) {
			...
			if(panResponse.isResponseOK()) {
				ProfessionalDetail professionalDetail = professionalDetailService.findById(entity.getProfessionalDetail().getId());
				professionalDetail.setPanVerified(true);
				professionalDetailService.save( professionalDetail);
			}
		}
	}

	public String createCrmEntity(Long userDetailId) {
		UserDetail entity = findById(userDetailId);
		//TODO update status to user detail
		String leadKey = crmNextDAO.createSave(entity);
		//TODO update lead id in user detail
		return leadKey;
	}

	private void applicaitonNotification(Long userDetailId) {
		UserDetail entity = findById(userDetailId);
		Map<String,String> data = new HashMap<>();
		data.put("refno", entity.getCrmLeadId());//TODO confirm if ref number to be sent is lead id or not. 
		...
		if(BRE_STATUS_GREEN.equalsIgnoreCase(entity.getBreStatus())) {
		...
		} else if (BRE_STATUS_AMBER.equalsIgnoreCase(entity.getBreStatus())) {
			smsService.send(entity.getMobile(), smsTemplateService.getFormattedText(3l, data));
			EmailTemplate emailTemplate = emailTemplateService.getById(2l);
			smtpMailSender.send(entity.getEmail(),emailTemplate.getSubject(), emailTemplateService.getFormattedText(emailTemplate ,data ));
		} else if (BRE_STATUS_RED.equalsIgnoreCase(entity.getBreStatus())) {
    ...
    @Override
    public Map<String,Object> verifyAadhaarOtp(Long userDetailId, AadhaarOtpKeyRequest otpRequestToBeSent) {
    	 	UserDetail userDetail = findById(userDetailId);
		...
		try {
			...
			mapToReturn.put("status", otpStatus);
			if(otpStatus) {
		    	 	userDetail.setAadhaarVerified(otpStatus);
		    	 	
		    	 	String responseStr = new String(aadhaarOtpKeyResponse.getUidData().getBytes());
		    	 	mapToReturn.put("data", extractData(responseStr));
		    	 	save(userDetail);
    ...
    private Map<String,String> extractData(String uidData) throws JAXBException {
		String responseStr =  new String(Base64.getDecoder().decode(uidData.getBytes()));
		responseStr = responseStr.substring(responseStr.indexOf("<kycRes>") + 8, responseStr.indexOf("</kycRes>") );
		responseStr = new String(Base64.getDecoder().decode(responseStr.getBytes()));
		responseStr = responseStr.substring(responseStr.indexOf("<UidData") , responseStr.indexOf("</UidData>") + 10);
		
		Map<String,String> aadhaarData = new HashMap<>();
		
		extractField(responseStr, aadhaarData, "dob");
		extractField(responseStr, aadhaarData, "gender");
		extractField(responseStr, aadhaarData, "house");
		extractField(responseStr, aadhaarData, "lm");
		extractField(responseStr, aadhaarData, "loc");
		extractField(responseStr, aadhaarData, "pc");
		extractField(responseStr, aadhaarData, "street");
		extractField(responseStr, aadhaarData, "subdist");
		extractField(responseStr, aadhaarData, "vtc");
		extractField(responseStr, aadhaarData, "state");
		extractField(responseStr, aadhaarData, "po");
		extractField(responseStr, aadhaarData, "dist");
		extractField(responseStr, aadhaarData, "country");
		
		return aadhaarData;
	}

	private static void extractField(String responseStr, Map<String, String> aadhaarData, String fieldName ) {
		
		String value = "";
		
		try {
			value = responseStr.substring(responseStr.indexOf(fieldName + "=\"") + fieldName.length() + 2 , responseStr.indexOf(fieldName+ "=\"") + fieldName.length() + 2 +  responseStr.substring(responseStr.indexOf(fieldName + "=\"") + fieldName.length() + 2 ).indexOf("\"") );
		} catch (Exception e) {
			//field not found.. ignore
		}
		
		aadhaarData.put(fieldName, value);
	}
    ...
    private OtpRequest createOtpRequest(UserDetail userDetail) {
	    	...
	    	otpRequest.setUniqueRefernceNumber(""+userDetail.getId() + DateUtil.getDateCPString(userDetail.getCreatedOn()));
	    	otpRequest.setUserId("OLP"+userDetail.getId());
	    	otpRequest.setDeviceCreatedOn(""+userDetail.getId() + DateUtil.getDateCPString(userDetail.getCreatedOn()));
